...
def preprocessing_data(input, task, model_name, tokenizer, device):
  if task == "fill-mask":
    ...
    input_tensor = tokenizer.encode(processed_input,
...
def perform_inference(model, task, inputs):
  with torch.no_grad(
  ):  # ensures no gradient computations are performed (not training)
    if task == "fill-mask":
...
def query_just_data(tokenizer, task, input_data, device, model_name):
  inputs_pre = preprocessing_data(input_data, task, model_name, tokenizer,
                                  device)
  return inputs_pre
...
def main():
  ...
  if task_type == "fill-mask":
    models_list = [
        "xlm-roberta-large", "bert-large-uncased", "roberta-base",
        "bert-base-uncased", "distilbert-base-uncased", "albert-base-v2"
    ]
  else:
    models_list = ["gpt2"]
  ...
  for model_name in models_list:
    ...
    if interface == "pipeline":
      pipe = load_pipeline(task=task_type,
                           model=model_name,
                           tokenizer=tokenizer,
                           device=device)
    ...
    for _i in tqdm(range(10)):
      if interface == "manually-just-inference":
        ...
        query_just_inference_extract(tokenizer, task_type,
                                     first_input_data_proc, model)
        ...
        for input_data in dataset:
          ...
          if interface == "manually":  #calls query 2: processing + inference + answer extraction
            query_2(tokenizer, task_type, input_data, model, device,
                    model_name)
